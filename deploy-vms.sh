...
for node_type in $(get_all_types); do
  for node in $(get_host_type ${node_type}); do
    ...
    sed -i "s|br-dhcp|br-pxe|g" /etc/libvirt/qemu/${node%%":"*}.openstackci.local.xml
    sed -i "s|vm-br-eth1|br-mgmt|g" /etc/libvirt/qemu/${node%%":"*}.openstackci.local.xml
    sed -i "s|vm-br-eth2|br-flat|g" /etc/libvirt/qemu/${node%%":"*}.openstackci.local.xml
    sed -i "s|vm-br-eth3|br-vlan|g" /etc/libvirt/qemu/${node%%":"*}.openstackci.local.xml
    sed -i "s|vm-br-eth4|br-vxlan|g" /etc/libvirt/qemu/${node%%":"*}.openstackci.local.xml
    sed -i "s|vm-br-eth5|br-storage|g" /etc/libvirt/qemu/${node%%":"*}.openstackci.local.xml
  done
done
